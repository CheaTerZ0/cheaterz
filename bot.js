...
member.guild.fetchInvites().then(guildInvites => {
    ...
    const yumz = member.guild.channels.find("name", `${sChannel}`);
     yumz.send(`- User : <@${member.user.id}> joined Vengance Empire by <@${inviter.id}>`);
...
client.on('messageUpdate', (oldRebel, newRebel) => {
   ...
   if (newRebel.content.toUpperCase().match(/DISCORD.GG/i))
    {
        console.log(newRebel.author.name + " Try posting by modifying the message - " + newRebel);
           newRebel.delete().catch(O_o=>{}); 
           newRebel.author.send("Discord links are Forbidden. \ nIf you want to publish, proceed from management.");
    }
});
